angular.module('workApp').controller('RegisterController', function($http, $rootScope, $scope, $state,$cookieStore,$location,RegisterService,ToastFactory) {
	...
	$scope.checkEmail = function(){
		if($scope.Credential.email){
			...
			$http({
			  	method: 'GET',
			  	url: '/api/user/checkEmail/' + $scope.Credential.email,
	...
	$scope.checkPhone = function(){
		if($scope.Credential.phone){
			...
			$http({
			  	method: 'GET',
			  	url: '/api/user/checkPhone/' + $scope.Credential.phone,
			}).success(function(response){
				if(response.status){
